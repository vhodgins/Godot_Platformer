...
func _ready():
	if grandparent == active_en:
		var packed_en = [self, get_parent().position, get_parent().scale]
		enemylist.append_inst(packed_en)
...
func movement_generator():
	...
	if physics_enabled and is_on_floor():
		if abs(velocity.x)<500:
			velocity.x += move_speed
		if on_right and (velocity.x>0):
			move_speed = -move_speed
		elif on_left and (velocity.x<0):
			move_speed = -move_speed
#	
